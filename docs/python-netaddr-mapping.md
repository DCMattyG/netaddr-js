...
net = IPNetwork('192.0.2.11/24', flags=NOHOST)
str(net)  # 192.0.2.0/24
```
...
const net = new IPNetwork('192.0.2.11/24', null, NOHOST);
String(net); // 192.0.2.0/24
```
...
- Python usually passes flags by keyword (`flags=...`).
- JavaScript passes flags as the third constructor argument.
- If you want auto-detected version in JavaScript, pass `null` for the second argument.
- `INET_ATON` and `INET_PTON` are mutually exclusive, matching Python `netaddr`.

Python to JavaScript flag mapping examples:

```python
IPAddress('127.1', flags=INET_ATON)
IPNetwork('192.0.2.11/24', flags=NOHOST)
```

```js
new IPAddress('127.1', null, INET_ATON);
new IPNetwork('192.0.2.11/24', null, NOHOST);
```

Combined flags use bitwise OR in both ecosystems:

```python
IPAddress('010.000.000.001', flags=INET_ATON | ZEROFILL)
```

```js
new IPAddress('010.000.000.001', null, INET_ATON | ZEROFILL);
```

## Common migration gotchas

- `IPNetwork('192.0.2.11/24')` preserves host bits in text form.
- Use `.cidr` or `NOHOST` to normalize to `192.0.2.0/24`.
- Replace Python `int(ip)` patterns with `ip.toBigInt()` in JavaScript.
- JS iterators are lazy and one-pass.
- Convert with `Array.from(...)` when you need reusable values.
- JavaScript classes require `new` for construction.
